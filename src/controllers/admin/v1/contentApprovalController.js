      ...
      contentApprovalListValidation(reqParam, res, async (validate) => {
        ...
        if (validate) {
          ...
          const filterCondition = {
            deletedAt: null,
            company_id: reqParam.companyId ? toObjectId(reqParam.companyId) : null,
            ...(reqParam.searchKey && {
            ...
            ...(reqParam.contentType && { content_type: parseInt(reqParam.contentType) }),
      ...
      contentApprovalValidation(reqParam, res, async (validate) => {
        if (validate) {
          ...
          if (updatedData) {
            ...
            if (reqParam.contentStatus === CONTENT_STATUS.APPROVED) {
              ...
              switch (updatedData.content_type) {
                case CONTENT_TYPE.FOCUS:
                  if (updatedData.parentId) {
                    const latestData = await Focus.findOne({
                  ...
                  await Focus.bulkWrite([
                    {
                      updateOne: {
                        filter: {
                          _id: updatedData.parentId
                            ? updatedData.parentId
                            : updatedData.content_type_id
                        },
                        update: updateData
                      }
                    },
                ...
                case CONTENT_TYPE.SURVEY:
                  ...
                  break;
                case CONTENT_TYPE.IDEA:
                  contentName = 'Idea';

                  if (updatedData.parentId) {
                    const latestData = await Idea.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
                ...
                case CONTENT_TYPE.AFFIRMATION:
                  contentName = 'Affirmation';
                  if (updatedData.parentId) {
                    const latestData = await Affirmation.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
                ...
                case CONTENT_TYPE.MEDITATION:
                  ...
                  if (updatedData.parentId) {
                    const latestData = await Meditation.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
                ...
                case CONTENT_TYPE.SOUND:
                  contentName = 'Sleep Sound';
                  if (updatedData.parentId) {
                    const latestData = await Sound.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
                ...
                case CONTENT_TYPE.GRATITUDE:
                  contentName = 'Gratitude';
                  if (updatedData.parentId) {
                    const latestData = await Gratitude.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
                ...
                case CONTENT_TYPE.RITUALS:
                  contentName = 'Shoorah Rituals';
                  if (updatedData.parentId) {
                    const latestData = await Ritual.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
                ...
                case CONTENT_TYPE.SHOORAH_PODS:
                  contentName = 'Shoorah Pods';
                  if (updatedData.parentId) {
                    const latestData = await ShoorahPods.findOne({
                      parentId: updatedData.parentId
                    }).select('-_id -parentId -approved_by -approved_on');
                    updateData = {
                      ...updateData,
            ...
            const notifyObj = {
              title: CONTENT_APPROVAL_TITLE,
              message:
                req.authAdminName +
                (reqParam.contentStatus === CONTENT_STATUS.APPROVED
                  ? CONTENT_APPROVAL_MESSAGE.APPROVED() + ' - ' + contentName
                  : reqParam.contentStatus === CONTENT_STATUS.REJECTED
                    ? CONTENT_APPROVAL_MESSAGE.REJECTED() + ' - ' + contentName
                    : CONTENT_APPROVAL_MESSAGE.DRAFT() + ' - ' + contentName),
            ...
            const newNotifyData = await createContentApprovalStatusNotification(notifyObj);
  ...
  getContentDetails: (req, res) => {
    try {
      ...
      getContentDetailsValidation(reqParam, res, async (validate) => {
        if (validate) {
          reqParam.contentType = parseInt(reqParam.contentType);
